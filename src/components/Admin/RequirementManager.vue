...
const emit = defineEmits<{
  ...
  (event: 'requirements-loaded', payload: Requirement[]): void
}>()
...
async function saveRequirement() {
  ...
  try {
    ...
    const providerKey = Number(props.providerId)
    const providerRef = Number.isNaN(providerKey) ? props.providerId : providerKey
    ...
    if (reqIsEdit.value && reqCurrentId.value) {
      ...
      const { error } = await supabase.from('subsidy_requirements').insert([payload])
      if (error) throw error
...
async function deleteRequirement() {
  if (!reqDeleteId.value || !props.providerId) return
  try {
    const { error } = await supabase
      .from('subsidy_requirements')
      .delete()
...
function subscribeRealtime() {
  if (requirementsChannel.value) {
    supabase.removeChannel(requirementsChannel.value)
    requirementsChannel.value = null
  }
  if (!props.providerId) return

  requirementsChannel.value = supabase
    .channel(`requirements-${props.providerId}`)
    .on(
      'postgres_changes',
      {
        event: '*',
...
<template>
  <v-card class="mb-6" elevation="8">
    <v-card-title class="d-flex align-center">
      ...
      <v-chip v-if="requirements.length" size="x-small" color="primary" variant="flat" class="ms-3">
  ...
  <v-dialog v-model="confirmReqDeleteOpen" max-width="420">
    <v-card>
      ...
      <v-card-text>
        Deleting this requirement removes it from all linked rules. Continue?
      </v-card-text>
      <v-card-actions>
        <v-spacer />
